...
function initRightupRegion() {
    ...
    gAjaxPost.aysncPostFormData = function(url, formData, callback) {   // formData is FormData object    // make it jquery style
        $.ajax({
            url: url,
            type: "POST",
            processData: false,
...
function updatePlayerStatus(player, status) {
    ...
    $('#' + player + '-send2main-btn').css({ 'display': 'inline' })
}
...
function sendToMainScreen(player) {
    try {
        var currentPlayer = RightUp['player' + player];
